...
const ProjectItem = ({ item }) => {
  return (
    <div className="w-full m-auto md:grid grid-cols-3 gap-8 mb-20">
      <div className="col-span-2">
        <h2 className="py-1">{item.project_name}</h2>
        <p className="text-gray-500">{item.project_period}</p>
        <p className="my-3 text-[25px] text-gray-500">{item.project_desc}</p>
        <div>
          {item.project_info.map((els, index) => {
            return (
              <div className="text-xl" key={`info_${index}`}>
                - {els.info}
              </div>
            );
          })}
        ...
        <div>
          ...
          {item.project_tech.map((el, index) => {
            return (
              <div className="text-2sm text-gray-600/75" key={`tech_${index}`}>
                - {el.tech}
              </div>
            );
          })}
